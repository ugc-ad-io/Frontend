...
function LogoModel({ progress, theme, idleSpin = true, verticalSpin = false }) {
  ...
  useFrame((state) => {
    ...
    if (!meshesRef.current) {
      ...
      scene.traverse((o) => {
        if (o.isMesh) {
          // No HDRI env anymore (see below) — so drop envMapIntensity and lean on a stronger
          // emissive + the scene lights. Much cheaper per fragment (no env sampling) → smooth scroll.
          o.material = new THREE.MeshStandardMaterial({ metalness: 0.1, roughness: 0.6, envMapIntensity: 0 });
...
export default function HeroLogo3D({ progress, theme, idleSpin = true, verticalSpin = false }) {
  return (
    <Canvas
      ...
      // Render resolution. Desktop renders at 1x (no retina super-sampling). Mobile (verticalSpin)
      // renders at 0.7x — the mark is small + decorative, so the slight softness is imperceptible,
      // but ~half the fragments per frame roughly halves the GPU cost, which is the mobile lag.
      dpr={verticalSpin ? 0.7 : 1}
    ...
    >
      ...
      <Suspense fallback={null}>
        ...
        {/* HDRI environment removed — sampling a 1K HDR per fragment every frame was the heaviest
            per-frame cost and the main scroll lag. The emissive self-lit material + scene lights
            carry the look. */}
      </Suspense>
    </Canvas>
  );
