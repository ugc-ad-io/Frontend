  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        /* The collapsed rail is 76px wide with 14px padding, so its content box is 48px, and
           .hsr-brand's 8px left pad leaves 40px. A fixed 44px mark did not fit and the
           rail's overflow:hidden sliced it - the logo showed as a thin bar in the corner.
        ...
        /* Collapsed: the MARK only. .hsr-label is already display:none at this width, so
           the "UGCad.io" wordmark beside it drops out on its own and the icon is all that
           is left - which is the intent.
           The sizing is the part that has to be exact. The rail is 76px wide with 14px
           padding, so the content box is 48px, and .hsr-brand's 8px left pad left only
           40px for a mark declared at 44px. It overflowed and the rail's overflow:hidden
           sliced it into a vertical sliver. Zero the pad, centre it, and size the mark to
           40px so it sits inside 48px with room either side. */
